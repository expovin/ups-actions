...
module.exports = {
    ...
    chkUPSStatus : function() {
        let result={};
        return new Promise( (fulfill, reject) =>{
            console.log("Checking UPS Status : "+settings.command.getUPSStatus);
            ...
            .then(stdout =>{
                ...
                stdout.split("\n").forEach((ele,idx) => {
                    if(idx > 0){
                        ...
                        let val=ele.split(':')[1];
                        console.log("key --> "+key+" key1 --> "+key1+" key2 --> "+key2);
                        if(key1){
                            if(!result[key1.trim()])
                                result[key1.trim()]={}
...
/* */
    sendWebHookToHomeAssistant : function(code, script) {
        ...
        let uri = settings.HomeAssistant.url+
                    ...
                    settings.HomeAssistant.code;

        let body = {code : code, script:script}
